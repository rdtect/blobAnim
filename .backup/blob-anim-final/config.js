...
(() => {
  ...
  if (typeof window.BlobConfig === "undefined") {
    const ConfigManagerInstance = (() => {
      ...
      class ConfigManagerClass {
        ...
        subscribe(callback) {
          ...
          return () => {
            ...
            if (this.config.debug) {
              console.log(
                `Subscriber removed (${this.subscribers.length} remaining)`
              );
            }
          };
        }
        ...
        notifySubscribers(oldConfig, newConfig) {
          ...
          if (this.config.debug) {
            console.log(
              `Notifying ${this.subscribers.length} subscribers of config change`
            );
          }
      ...
      return {
        getInstance: (initialConfig) =>
          ConfigManagerClass.getInstance(initialConfig),
      };
    })();
    ...
    window.BlobConfig = {
      getConfigManager: (initialConfig) =>
        ConfigManagerInstance.getInstance(initialConfig),
